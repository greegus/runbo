...
Update prompt: non-blocking toast "A new version is available" with an Update button. No install banner
in MVP — the user installs from the browser menu.
...
| 9 | PWA — `vite-plugin-pwa`, manifest, icons, shell caching | Lighthouse: installable; DevTools offline → app loads, a logged session syncs on reconnect | ⬜ |
